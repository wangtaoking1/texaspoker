...
//		}
	...
	public String thinkAfterHold(ArrayList<BetState> betStates) {
		...
		int maxBlindBet = Constants.LESS_HIGH_BET_MULTIPLE * this.getBlind(); // 可接受（跟注）最大下注筹码
		int midBlindBet = Constants.LESS_MIDDLE_BET_MULTIPLE * this.getBlind(); // 可接受（跟注）中等下注筹码
		int lowBlindBet = Constants.LESS_LOW_BET_MULTIPLE * this.getBlind();

		// 如果手牌是大对：AA, KK, QQ, JJ, 1010等
		if (this.isHoldBigPair(hp)) {
			// 加注至 MIDDLE_RAISE_MULTIPLE * blind
			return raiseByDiff(diff, Constants.LESS_HIGH_RAISE_MULTIPLE); // TODO
		}
		// 手牌是小对：2~9中的一对
		...
		// 手牌都小于10
			...
			// 其它牌型，50%概率跟注
			else if ((int) Math.random() * 100 <= Constants.LESS_CALL_PERCENTAGE) {
				if (diff <= lowBlindBet)
					return callByDiff(diff);
			}
		}
	...
	public String thinkAfterFlop(ArrayList<BetState> betStates) {
		...
		if (power > (long) 2 * Math.pow(10, 10)
			...
			// 手牌是大对，加中倍注
			if (isHoldBigPair(hp))
				return raiseByDiff(diff, Constants.LESS_MIDDLE_RAISE_MULTIPLE);
			// 手牌是小对，加低倍注
				...
				// 说明三条是出现在公共牌里
					...
					if (hp.get(0).getValue() >= Constants.GAP_VALUE
						...
						if (diff <= Constants.LESS_MIDDLE_BET_MULTIPLE
								* this.getBlind())
							return callByDiff(diff);
					} else if (diff <= Constants.LESS_LOW_BET_MULTIPLE
							* this.getBlind())
						return callByDiff(diff);
				}
			}
		...
		// 同花或顺子差一张
			...
			if (diff <= Constants.LESS_MIDDLE_BET_MULTIPLE * this.getBlind())
				return callByDiff(diff);
		}
		// 高牌
		else if (this.isHoldBig(hp)) {
			if (diff <= Constants.LESS_LOW_BET_MULTIPLE * this.getBlind())
				return callByDiff(diff);
		} 
		// 手牌中有一张是大牌
		else if (hp.get(0).getValue() >= Constants.GAP_VALUE
				|| hp.get(1).getValue() >= Constants.GAP_VALUE) {
			if (diff <= Constants.LESS_LOW_BET_MULTIPLE * this.getBlind()
					&& (int) (Math.random() * 100) <= Constants.LESS_CALL_PERCENTAGE) // 按照一定的概率跟注
				return callByDiff(diff);
		} else if (diff == 0)
			return "check";
		return fold(betStates);
	...
	public String thinkAfterRiver(ArrayList<BetState> betStates) {
		...
		if (power > (long) 2 * Math.pow(10, 10)
			...
			if (isHoldBigPair(hp))
				return raiseByDiff(diff, Constants.LESS_LOW_RAISE_MULTIPLE);
			// 手牌是小对，跟注
			else if (isHoldSmallPair(hp) && diff <= Constants.LESS_HIGH_BET_MULTIPLE * this.getBlind()) {
				return callByDiff(diff);
			} else {
				ArrayList<Integer> pubPair = this.getPubPairValue(pp); // 获取公共牌中的对子的值
				// 公共牌中有一对，说明手牌没有和公共牌中的某一张组成对子 ，这种情况跟高牌差不多
				if (pubPair.size() == 1) {
					...
					if (hp.get(0).getValue() >= Constants.GAP_VALUE
							|| hp.get(1).getValue() >= Constants.GAP_VALUE) {
						if (diff <= Constants.LESS_LOW_BET_MULTIPLE
								* this.getBlind())
							return callByDiff(diff);
					}
				}
				// 说明手牌中的一张牌与公共牌中的一张牌组成对子
					...
					if (pairValues.get(0) >= Constants.GAP_VALUE)
						return raiseByDiff(diff,
								Constants.LESS_LOW_RAISE_MULTIPLE);
					// 小对，加低倍注
					else if (diff <= Constants.LESS_HIGH_RAISE_MULTIPLE * this.getBlind())
						return callByDiff(diff);
				}
			}
		}
